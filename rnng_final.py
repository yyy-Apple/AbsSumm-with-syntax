...
class TransitionParser(nn.Module):
    ...
    def __init__(self, word_vocab: Vocab, act_vocab: Vocab, nt_vocab: Vocab):
        ...
        self.stack_lstm = nn.LSTMCell(input_size=LSTM_DIM,
                                      hidden_size=LSTM_DIM)
    ...
    def do_action(self, action: int, open_nts: List[int], n_terms: int, final_states, train_sent=None):
        ...
        if self.act_vocab.i2w[action] == 'SHIFT':
            ...
            h0, c0 = self.stack[-1][0]
            h1, c1 = self.stack_lstm(word_embedding, (h0, c0))
            self.stack.append(((h1, c1), gold_word))
            ...
            h0, c0 = self.stack[-1][0]
            h1, c1 = self.stack_lstm(nt_embedding, (h0, c0))
            self.stack.append(((h1, c1), "(" + self.nt_vocab.i2w[nt_index]))
